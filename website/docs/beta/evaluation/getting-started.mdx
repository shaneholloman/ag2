...
Keep each scorer to a single, specific question — three small checks tell you *what* broke when one fails; one big check just says "something's wrong." See [Scorers](/docs/beta/evaluation/scorers/) for the prebuilts (including the `agent_judge` LLM scorer) and the return-type rules.
...
- **[Scorers](/docs/beta/evaluation/scorers/)** — the six prebuilts, writing custom scorers, and `agent_judge` for grading subjective quality.
- **[Runs](/docs/beta/evaluation/runs/)** — datasets and the `RunResult` in depth, grading existing traces (`evaluate_traces`), and observing a run live.
...
- **[Persistence & tracking](/docs/beta/evaluation/persistence/)** — save runs and compare them over time to catch regressions and confirm improvements.
