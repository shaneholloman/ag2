...
- **`pairwise_judge(config, criterion=, key=)`** — an LLM. It judges each pair in **both orders** (A-then-B and B-then-A) and only counts a win when the verdict is consistent, which cancels position bias.
...
- **[Variants](/docs/beta/evaluation/variants/)** — rank more than two on a leaderboard.
- **[Scorers](/docs/beta/evaluation/scorers/)** — `pairwise_judge` is the head-to-head cousin of the `agent_judge` scorer.
