...
!!! note
    `ImageGenerationTool` is only supported by OpenAI (Responses API). Gemini generates images through a response modality rather than a tool. See [Image Generation](/docs/beta/multimodal/image_generation/) for the full guide covering both providers.
