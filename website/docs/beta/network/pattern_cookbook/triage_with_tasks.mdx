...
* **`#!python knobs["context_vars"]` seeds state at session creation.**
  Any tool / middleware can read it via `#!python SessionStateInject`;
  ...
  demonstrate that session-scoped state survives the entire run.
...
| Classic | Beta |
|---|---|
| `#!python ReplyResult(context_variables={"current_task_type": ..., "pending_tasks": [...]})` | `#!python set_context(session, key, value)` per field (dynamic variant) |
| `#!python OnContextCondition` per task type | `#!python ContextEquals("current_task_type", <type>)` per branch (dynamic variant) |
| Initial `#!python ContextVariables(data=...)` passed to the pattern | `#!python knobs["context_vars"]` on `#!python session.open(...)` |
