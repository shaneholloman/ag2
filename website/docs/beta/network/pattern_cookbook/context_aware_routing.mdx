...
that calls `#!python set_context(session, "category", <category>)`. The
...
| `#!python ReplyResult(context_variables={"category": "billing"}, target=AgentTarget(billing_specialist))` | `#!python await set_context(session, "category", "billing")` from inside a plain `#!python @tool`; `#!python ContextEquals("category", "billing") → AgentTarget(billing)` graph rule |
