...
* **No parallel dispatch.** Classic Redundant could fan out to three
  ...
  sessions opened by the asker's tool, gather replies via
  `#!python wait_for_session_event`, then write the results back into
...
* **No `#!python NestedChatTarget`.** Same as Hierarchical — child
  sessions are the workaround when you genuinely want isolated
  specialist runs.
