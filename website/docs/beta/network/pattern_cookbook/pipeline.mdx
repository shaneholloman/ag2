...
| `#!python ContextVariables` carrying intermediate state | `#!python set_context(session, key, value)` from inside a tool; reads via `#!python SessionStateInject` |
...
async def main() -> None:
    ...
    await session.send("Order: 2x widget (SKU W-100), customer ACME-7, ship-to: London EC1.")
