...
!!! note
    Both patterns share the same audio I/O primitives — [`SoundDevicePlayer`](/docs/beta/live/stt_tts) and [`SoundDeviceRecorder`](/docs/beta/live/stt_tts) — and the same event stream. You can mix observers (TTS, logging, persistence) across both.
...
- **[STT & TTS](/docs/beta/live/stt_tts)** — wrap any `Agent` with speech input and output.
- **[LiveAgent](/docs/beta/live/live_agent)** — realtime, low-latency voice agents with OpenAI or Gemini.
