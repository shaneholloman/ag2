...
```python linenums="1" hl_lines="13-14 18-21 23-25 27"
...
agent = Agent(
    "assistant",
    prompt="Use the get_weather tool when asked about weather.",
    config=OpenAIConfig("gpt-4o-mini", streaming=True),  # streaming=True emits token chunks
    tools=[get_weather],
)

async with agent.run("What's the weather in Paris?") as run:
    ...
    @run.stream.where(ModelMessageChunk).subscribe()
    async def on_token(chunk: ModelMessageChunk) -> None:
...
* **Token streaming needs `#!python streaming=True` on the model config** (as above); the model then emits `ModelMessageChunk` events as it generates. Every event type is observed the same way — the example also filters `ToolCallEvent` to log tool calls live.
