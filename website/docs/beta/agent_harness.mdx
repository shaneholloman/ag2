...
See [Subagents](/docs/beta/subagents) for the full sub-task delegation guide — context flow and custom streams for self-delegation via `as_tool()`.
...
`as_tool()` returns a `FunctionTool` named `task_{child.name}` that accepts an `objective` parameter and forwards it into the child's stream. See [Subagents](/docs/beta/subagents) for sub-task streams, depth limiting, and stream factories.
