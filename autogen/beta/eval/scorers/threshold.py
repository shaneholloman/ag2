...
:class:`~autogen.beta.eval.Scorer` whose grade is a boolean pass/fail derived from that
...
A gated criterion emits ONE :class:`~autogen.beta.eval.Feedback`: ``score`` is the boolean
...
per-task token/time *resource* gate, see :class:`~autogen.beta.eval.BudgetThresholds` — a
different axis.
"""
